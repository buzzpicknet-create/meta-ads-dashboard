  ...
  async (req: Request, res: Response) => {
    try {
      ...
      const enriched = adsets.map((a) => {
        ...
        const hookRate = impressions > 0 && videoP3 > 0 ? ((videoP3 / impressions) * 100).toFixed(2) : null;
        return {
          ...a,
          insights: ins,
          spend: spend || null,
...
router.get("/pipeboard/campaigns/:id/ads", async (req: Request, res: Response) => {
  try {
    ...
    const normalized = await Promise.all(ads.map(async ad => {
      ...
      const hookRate = impressions > 0 && videoP3 > 0 ? Number(((videoP3 / impressions) * 100).toFixed(2)) : null;
