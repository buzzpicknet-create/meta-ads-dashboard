...
🔴 قاعدة عدد الـ AdSets — الـ backend يتبع adsets[] حرفياً:
  ┌─────────────────────────────────────────────────────────────────┐
  │ adsets[] فيها عنصر واحد  → مجموعة إعلانية واحدة               │
  │   + folder URL في creatives → كل فيديو يصبح إعلاناً مستقلاً   │
  │   النتيجة: 1 adset + N ads (واحد لكل فيديو)                   │
  ├─────────────────────────────────────────────────────────────────┤
  │ adsets[] فيها N عناصر   → N مجموعات إعلانية                   │
  │   + folder URL في creatives → كل فيديو يُوزَّع على adset منفصل │
  │   النتيجة: N adsets × M ads لكل منها                           │
  └─────────────────────────────────────────────────────────────────┘

مثال — 5 فيديوهات في مجموعة واحدة:
  adsets:    [{ name: "الاختبار", budget: 180 }]           ← عنصر واحد فقط
  creatives: [{ media_url: "drive_folder_url", ... }]      ← folder كامل
  النتيجة:  1 adset + 5 ads جوّاه

مثال — 5 فيديوهات كل واحد في مجموعة منفصلة (angles):
  adsets:    [{ name: "فيديو1" }, { name: "فيديو2" }, ...]  ← 5 عناصر
  creatives: [{ media_url: "drive_folder_url", ... }]
  النتيجة:  5 adsets × 1 ad لكل منها
...
خطوات التنفيذ:
١. حدد غرض الحملة: TESTING (ABO) أم SCALING (CBO)، وحدد طريقة البناء: STANDARD فقط إذا ذكر المستخدم Campaign Type: STANDARD.
٢. استخرج بدقة:
   ...
   - primary texts: لـ STANDARD = كل النصوص كـ array داخل creative.texts.
   - headlines: لـ STANDARD = كل العناوين بنفس الترتيب كـ array داخل creative.headlines.
   - budget: الرقم الكامل من "Budget:" أو "Campaign Budget:" — لا تغيره ولا تضع 20 افتراضياً.
   - daily_budget في launch_pipeboard_campaign = نفس الرقم بالضبط (مثال: "CBO · 300 EGP" → daily_budget=300).
   - media_url: لـ STANDARD = رابط مجلد Drive كما هو — لا تستدعِ upload_video_to_meta أو upload_ad_video مسبقاً — الـ backend يرفع تلقائياً.
٣. ابنِ adsets[] وcreatives[] لـ STANDARD بهذا الشكل:
   - adsets[]: كل Adset يحتوي name و budget و landing_page_url/link_url إن وُجدت.
   - creatives[]: لا تنشئ creative لكل فيديو. أنشئ creative واحد لكل Adset/Angle يحمل:
     { media_url: drive_folder_url, media_type: "video", texts: primary_texts[], headlines: headlines[], link_url: link_url الخاص بالـ Adset, name: اسم الـ Angle }
   - الـ backend داخل launch_pipeboard_campaign هو المسؤول عن توسيع Google Drive Folder إلى N فيديوهات وإنشاء N×M إعلانات منفصلة.
   - كل إعلان نهائي = فيديو واحد + نص واحد + عنوان واحد.
٤. ممنوع تماماً في STANDARD:
   - is_dynamic_creative
   - asset_feed_spec
   - creative_features_spec
   - instagram_actor_id
   - messages/headlines كـ multi-variant creative
٥. budget_type: استخرجه من سطر "budget_type (for launch_pipeboard_campaign):" — CBO أو ABO.
٦. إذا لم يكن account_id في الـ context: استدعِ fetch_account_metadata أولاً.
٧. استدعِ launch_pipeboard_campaign مرة واحدة فقط، ولا تستدعِ create_campaign أو create_adset أو create_ad بعدها.
    ...
    function: {
      ...
      parameters: {
        ...
        properties: {
          ...
          creatives: {
            type: "array",
            description: "مصفوفة creative specs. لـ STANDARD: لا تنشئ عنصر لكل فيديو. أنشئ عنصر واحد لكل Adset/Angle يحتوي media_url=Drive folder وtexts[] وheadlines[] وlink_url. الـ backend يوسّع Google Drive إلى N فيديوهات وينشئ N×M إعلانات منفصلة داخل نفس الـ Adset. ممنوع DCO أو asset_feed_spec.",
            items: {
              type: "object",
              properties: {
                ...
                media_url: { type: "string", description: "رابط الميديا — في STANDARD استخدم Google Drive Folder كما هو، والـ backend يكتشف كل الفيديوهات." },
