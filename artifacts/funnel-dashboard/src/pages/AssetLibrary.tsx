...
Media source URL (input only; expand to direct video_url in Blueprint): ${driveLink.trim() || "—"}
...
- أخرج Blueprint JSON فقط لكل فيديو × نص في نفس الـ Adset (${selAdset?.id ?? "???"}) — لا تنفّذ أي Meta API calls
- استخدم video_url مباشر لكل فيديو داخل Blueprint JSON فقط — لا تستخدم drive_folder_url ولا media_url كفولدر ولا upload_ad_video
...
⚠️ RULES (لا تخالف):
- لا تنشئ Adset فعلياً. أخرج Blueprint JSON فقط لإضافة Adset داخل الحملة ${selCampaign?.id ?? "???"}${isCBO ? " — CBO: لا تضع daily_budget على الـ Adset" : ` — ABO daily_budget: ${budget} EGP`} — لا تنفّذ create_adset ولا أي Meta API calls
- ZERO DCO · ZERO creative_features_spec · ZERO instagram_actor_id
- page_id / pixel_id يُكتشفان تلقائياً من الدومين
- أخرج Blueprint JSON فقط لكل فيديو × نص في الـ Adset الجديدة — لا تنفّذ أي Meta API calls
- استخدم video_url مباشر لكل فيديو داخل Blueprint JSON فقط — لا تستخدم drive_folder_url ولا media_url كفولدر ولا upload_ad_video
  ...
  function buildCmd(): string {
    ...
    const adsetBlocks = angles.filter(a => a.name.trim() || a.landing.trim()).map((a, i) => {
      ...
      const videoSection = hasVideoName
        ? `- Video: استخدم video_url المباشر للفيديو "${a.name.trim()}" فقط — لا تستخدم drive_folder_url ولا upload_ad_video\n- Ads (${creativesPerAdset} ads = 1 video x ${creativesPerAdset} copy pairs — NO DCO):`
        : `- Videos: استخدم video_url المباشر لكل فيديو فقط — لا تستخدم drive_folder_url ولا upload_ad_video — Total ads = N_videos x ${creativesPerAdset} copy pairs (NO DCO):`;
...
- Media source URL (input only; expand to direct video_url in Blueprint): ${drive}
# 2. الزوايا الإعلانية داخل Blueprint فقط (كل زاوية = AdSet blueprint + Ad blueprint — بدون تنفيذ على Meta)
${anglesSection}
# 3. إعدادات عامة
...
[END_COMMAND]`;
    ...
    return `[SYSTEM COMMAND: EXECUTE_CAMPAIGN_BLUEPRINT]
قم بتوليد Blueprint JSON فقط لحملة (SCALING) — لا تنشئ AdSet ولا إعلان فعلي على Meta:
# 1. Campaign Settings
  ...
  function buildStandardCmd(): string {
    ...
    const adsetBlocks = angles.length > 0
      ? angles.map((a, i) => {
          ...
          const hasVideoName = a.name.trim().length > 0;
          const videoSection = hasVideoName
            ? `- Video: استخدم video_url المباشر للفيديو "${a.name.trim()}" داخل Blueprint JSON فقط — لا تستخدم drive_folder_url ولا upload_video_to_meta\n- Ads (${creativesPerAdset} ads = 1 video × ${creativesPerAdset} copy pairs — NO Dynamic Creative):`
            : `- Videos: استخدم video_url المباشر لكل فيديو داخل Blueprint JSON فقط — Total ads = N_videos × ${creativesPerAdset} copy pairs (NO Dynamic Creative) — لا تستخدم drive_folder_url ولا upload_video_to_meta:`;
          return `## Adset ${i + 1} — "${a.name || `Angle ${i + 1}`}"
...
- Videos: استخدم video_url المباشر لكل فيديو داخل Blueprint JSON فقط — لا تستخدم drive_folder_url ولا upload_video_to_meta
...
Generate Standard Campaign Blueprint JSON ONLY — ${adsetCount} Adset(s):
...
- ZERO creative_features_spec — standard_enhancements مهمل من Meta ومرفوض على POST
...
- استخدم launch_pipeboard_campaign في وضع Blueprint-only فقط — لا ترفع فيديوهات، لا تنشئ حملة على Meta، ولا تنفّذ أي Meta API calls. أخرج Blueprint JSON جاهز فقط (STANDARD حقيقي — لا DCO)
...
[END_COMMAND]`;
  ...
  return (
    <div className="rounded-xl border border-border bg-card p-4 space-y-3">
      ...
      {/* ── Add Creative to Existing AdSet ── */}
      {activeCard === "ADD_EXISTING_ADSET" && (
        <AddCreativeExistingAdsetForm
