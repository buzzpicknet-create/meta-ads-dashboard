...
const QUICK_ACTIONS = [
  {
    ...
    prompt: "خطوات التنفيذ بدون سؤال: ١. get_campaign_daily(days=2) لجلب أرقام أمس (آخر يوم كامل — بيانات اليوم غير مكتملة). ٢. get_campaigns(days=7) للمقارنة بمتوسط الأسبوع. أعرض جدول: | الحملة | الإنفاق أمس | CPA أمس | CPA 7 أيام | Purchases أمس | الحالة | حيث الحالة: ✅ رابح (CPA أمس < 40 EGP) / ⚠️ تحت المراقبة (40-80 EGP) / 🔴 خطر (> 80 EGP أو 0 purchases بعد 200 EGP). ركّز على الحملات النشطة فقط.",
  },
  {
    label: "🚀 فرص الـ Scale",
    prompt: "حلل الحملات النشطة بناءً على أداء آخر 7 أيام. جيب الحملات عبر get_campaigns ثم استخدم get_campaign_budget لتحديد النوع (CBO/ABO). حدد الحملات والمجموعات التي تحقق CPA أقل من 40 EGP ومستقرة. قدّم مقترحات زيادة ميزانية 20% وولّد bulk_action مباشرةً بدون سؤال (ما عدا Aggressive Scale فوق 2×).",
  },
  {
  ...
  },
  {
    ...
    prompt: "جيب الحملات النشطة عبر get_campaigns(days=7). استخرج الحملات والمجموعات التي تجاوزت CPA > 60 EGP مع إنفاق > 100 EGP. استخدم get_adsets لتشخيص السبب (Frequency مرتفع؟ CTR منخفض؟). أعرض جدولاً بالكيانات المتراجعة ثم ولّد bulk_action لتقليل الميزانية 30% — استخدم update_campaign_budget للـ CBO وupdate_adset_budget للـ ABO.",
  },
  {
    label: "📊 مقارنة الأسبوعين",
    prompt: "جيب get_campaign_daily(days=14) لكل الحملات النشطة. قسّم البيانات: أسبوع 1 (الأيام 8-14) مقابل أسبوع 2 (الأيام 1-7). أعرض جدول مقارنة:\n| الحملة | CPA أسبوع 1 | CPA أسبوع 2 | التغيير% | Purchases أسبوع 1 | Purchases أسبوع 2 | الحكم |\nالحكم: 🟢 تحسّن / 🔴 تراجع / 🟡 ثابت. حدد الحملات التي تحتاج تدخلاً فورياً بناءً على الاتجاه.",
  },
] as const;
