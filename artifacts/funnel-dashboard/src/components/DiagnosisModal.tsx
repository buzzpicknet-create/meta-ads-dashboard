...
function buildCampaignContext(insights: CampaignInsights): string {
  ...
  const fmt2 = (n: number, d = 0) => n.toLocaleString("ar-EG", { maximumFractionDigits: d });
  ...
  const lines: string[] = [
    ...
    ``,
    `=== إجماليات الحملة ===`,
    `الإنفاق: ${fmt2(t.spend, 0)} EGP`,
    `الأوردرات: ${fmt2(t.purchases, 0)}`,
    `CPA: ${t.purchases > 0 ? fmt2(t.cpa, 0) + " EGP" : "لا تحويلات"}`,
    `الوصول (Reach): ${fmt2(t.reach, 0)}`,
    `الظهورات (Impressions): ${fmt2(t.impressions, 0)}`,
    `CPM: ${fmt2(t.cpm, 1)} EGP`,
    `CTR: ${pct(t.ctr)}`,
    `Hook Rate: ${pct(t.hookRate)}`,
    `ThruPlay Rate: ${pct(t.v100 / Math.max(t.video_plays, 1))}`,
    `Landing Page Rate: ${pct(t.lpvRate)}`,
    `CR (من LP): ${pct(t.crLpv)}`,
    `Frequency: ${t.frequency.toFixed(2)}`,
    `CPC: ${fmt2(t.cpc, 1)} EGP`,
    ``,
  ];

  if (insights.by_adset.length > 0) {
    lines.push(`=== Ad Sets (${insights.by_adset.length}) ===`);
    [...insights.by_adset]
      .sort((a, b) => b.spend - a.spend)
      .slice(0, 5)
      .forEach((s) => {
        lines.push(
          `• ${s.label} | إنفاق: ${fmt2(s.spend, 0)} EGP | أوردرات: ${s.purchases} | CPA: ${s.purchases > 0 ? fmt2(s.cpa, 0) + " EGP" : "-"} | Hook: ${pct(s.hookRate)} | CTR: ${pct(s.ctr)}`
        );
      });
    lines.push("");
  }
  ...
  if (insights.by_ad.length > 0) {
    lines.push(`=== الإعلانات الأعلى إنفاقاً (${Math.min(insights.by_ad.length, 5)} من ${insights.by_ad.length}) ===`);
    [...insights.by_ad]
      .sort((a, b) => b.spend - a.spend)
      .slice(0, 5)
      .forEach((a) => {
        lines.push(
          `• ${a.label} | إنفاق: ${fmt2(a.spend, 0)} EGP | أوردرات: ${a.purchases} | CPA: ${a.purchases > 0 ? fmt2(a.cpa, 0) + " EGP" : "-"} | Hook: ${pct(a.hookRate)} | CTR: ${pct(a.ctr)}`
        );
      });
  }
...
function AiChatTab({ insights }: { insights: CampaignInsights }) {
  ...
  return (
    <div className="flex flex-col h-full gap-2">
      {/* Messages area */}
      <div className="flex-1 overflow-y-auto space-y-3 pb-1 px-0.5">
            ...
            <div className="grid grid-cols-2 gap-2 w-full">
              {SUGGESTED.map((q) => (
                <button
                  ...
                  className="text-[11px] text-start px-3 py-2 rounded-lg border border-border bg-muted/30 hover:bg-muted/60 transition-colors leading-snug"
                >
                  {q}
                </button>
              ))}
        ...
        {messages.map((msg, i) => (
          <div key={i} className={`flex gap-2 ${msg.role === "user" ? "flex-row-reverse" : "flex-row"}`}>
            ...
            <div className={`text-xs leading-relaxed rounded-xl px-3 py-2.5 max-w-[85%] whitespace-pre-wrap ${msg.role === "user" ? "bg-primary text-primary-foreground rounded-tr-sm" : "bg-muted/60 border border-border rounded-tl-sm"}`}>
              {msg.content}
            </div>
          </div>
        ))}
        ...
        {streaming && streamingText && (
          <div className="flex gap-2 flex-row">
            ...
            <div className="text-xs leading-relaxed rounded-xl px-3 py-2.5 max-w-[85%] bg-muted/60 border border-border rounded-tl-sm whitespace-pre-wrap">
              {streamingText}
              <span className="inline-block w-1 h-3 bg-primary animate-pulse mr-0.5 rounded-sm" />
            </div>
          </div>
        )}
        ...
        {streaming && !streamingText && (
          <div className="flex gap-2">
            ...
            <div className="flex items-center gap-1 px-3 py-2.5 rounded-xl bg-muted/60 border border-border rounded-tl-sm">
              {[0, 1, 2].map((i) => (
      ...
      {/* Input area */}
      <div className="shrink-0 flex gap-2 items-end border-t border-border pt-2">
        {messages.length > 0 && (
          <button
            onClick={clearChat}
        ...
        <textarea
          ...
          disabled={streaming}
          className="flex-1 resize-none rounded-lg border border-border bg-muted/40 px-3 py-2 text-xs focus:outline-none focus:ring-1 focus:ring-primary placeholder:text-muted-foreground disabled:opacity-50 leading-relaxed max-h-24"
