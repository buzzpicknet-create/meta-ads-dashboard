...
import ProductHuntingPage from "@/pages/ProductHunting";
import { AuthProvider, useAuth } from "@/contexts/AuthContext";
import { useActivityLogger } from "@/hooks/use-activity-logger";
import { usePushNotifications } from "@/hooks/use-push-notifications";
import { Activity, LayoutDashboard, ClipboardList, Clapperboard, Sparkles, Settings, LogOut, Loader2, Bell, BellOff, Target, Search, Bot, Library, Package, ShieldAlert, AlertTriangle, Pause, X, CalendarDays, Globe, KeyRound, PackageSearch } from "lucide-react";
...
const ALL_NAV_ITEMS = [
  ...
  { href: "/product-hunting", label: "صيد المنتجات", Icon: PackageSearch,  useRoute: "/product-hunting", roles: ["admin", "media_buyer"] },
          ...
          >
            <div className="flex items-center justify-between px-4 py-3 border-b border-border bg-red-50 dark:bg-red-950/20">
              <div className="flex items-center gap-2">
                ...
                <span className="text-sm font-semibold text-red-600 dark:text-red-400">المراقب الذكي</span>
              </div>
              <button
            ...
            <div className="max-h-[420px] overflow-y-auto divide-y divide-border">
              {notifications.length === 0 ? (
                <div className="px-4 py-8 text-center text-sm text-muted-foreground">لا توجد تنبيهات حالياً</div>
              ) : (
                notifications.map((n) => (
...
function NavBar() {
  ...
  return (
    <>
              ...
              {navItems.map((item) => {
                ...
                return (
                  <Link
                    key={item.href}
                    href={item.href}
                    className={`inline-flex items-center gap-2 rounded-lg px-3 py-1.5 text-sm font-medium transition-colors whitespace-nowrap ${
                      active
      ...
      <nav
        className="sm:hidden fixed bottom-0 inset-x-0 z-50 border-t border-border bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 overflow-x-auto"
        dir="rtl"
      >
        <div className="flex items-center h-16 px-1 min-w-max">
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`flex flex-col items-center justify-center gap-0.5 w-20 h-full rounded-xl transition-colors ${
          ...
          <button
            onClick={logout}
            className="flex flex-col items-center justify-center gap-0.5 w-20 h-full rounded-xl transition-colors text-muted-foreground"
          >
  ...
  return (
    <GlobalAiChatContext.Provider value={ctxValue}>
      ...
      <Switch>
        ...
        <Route path="/product-hunting" component={canAccess("/product-hunting") ? ProductHuntingPage : NotFound} />
